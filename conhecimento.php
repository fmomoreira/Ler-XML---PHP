       ...
       var peganomexml = document.querySelector("#exampleFormControlSelect1");
        ...
        function sucesso(arquivo, destinatario) {
           // console.log($(arquivo).find('prod').find('cEAN').text());
            ...
            elemento += "<thead>";
            elemento += "<tr>";
            elemento += "<th scope='coll'>CNPJ</th>";
            elemento += "<th scope='coll'>Transportadora</th>"
            elemento += "<th scope='coll'>Endereço</th>"
            elemento += "<th scope='coll'>Bairro</th>"
            elemento += "<th scope='coll'>Municipio</th>"
            elemento += "<th scope='coll'>Estado</th>"
            elemento += "<tr>";
            elemento += "</thead>";
            elemento += "<tbory>"
            $(arquivo).find('infCte').each(function() {
                var cnpj = $(this).find('emit').find('CNPJ').text();
                var nome = $(this).find('emit').find('xNome').text();
                var ruaentrega = $(this).find('dest').find('enderDest').find('xLgr').text();
                var bairro = $(this).find('dest').find('enderDest').find(' xBairro').text();
                var municipio = $(this).find('dest').find('enderDest').find('xMun').text();
                var estado = $(this).find('dest').find('enderDest').find('UF').text();
                
                elemento += "<tr>";
                elemento += "<td >" + cnpj +  "</td>" ;
                elemento += "<td >" + nome +  "</td>" ;
                elemento += "<td >" + ruaentrega +  "</td>" ;
                elemento += "<td >" + bairro +  "</td>" ;
                elemento += "<td >" + municipio +  "</td>" ;
